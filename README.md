...
```
python run_custom.py --maze mazes/maze_8x8_1.txt --agent dqn --encoding onehot --episodes 300
```
...
DQN results (larger maze + the coordinate-vs-one-hot state-representation comparison) are in `results/sweep_summary.txt` — see that file for the current numbers, since it's regenerated by `tools/run_sweep.py`.

**Known limitation:** DQN with plain epsilon-greedy exploration does not converge on `maze_15x15.txt` or `maze_20x20.txt` within a feasible episode budget (confirmed: flat at the step cap for the full 300-episode run). This isn't a bug — a random walk's expected time to find the goal scales with the number of free cells (449 on `maze_15x15.txt`, vs. 127 on the mazes where DQN does converge), not the optimal path length, and fixing it would need a smarter exploration strategy (e.g. reward shaping or count-based exploration) that's outside this project's scope.